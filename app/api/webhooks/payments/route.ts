  ...
  try {
    ...
    if (payload.customer?.email && process.env.RESEND_AUDIENCE_ID) {
      try {
        ...
        console.log(
          `[Webhook] Cliente agregado a la Audiencia de Resend exitosamente.`,
        );
      } catch (e) {
    ...
    if (payload.event === 'payment_pending') {
      ...
      const baseUrl =
        process.env.NODE_ENV === 'development'
          ? `${payload.tenant}.localhost:3001`
          : `${payload.tenant}.${domain}`;
      const adminUrl = `${protocol}://${baseUrl}/admin/payments?paymentId=${payload.paymentId}`;

      // Notificar al admin
      await resend.emails.send({
        from: 'Notificaciones Tienda <noreply@todoesunbalance.com>',
        to: 'todoesunbalance@gmail.com',
        ...
        react: AdminNewOrderEmail({
          ...
          paymentReference:
            ...
            payload.metadata?.paymentReference ||
            'N/A',
          adminUrl,
        }) as React.ReactElement,
      });
      ...
      console.log(
        `[Webhook] Evento pending: Correos enviados al admin y al cliente.`,
      );
          ...
          if (product) {
            if (product.type === 'digital' && product.fileName) {
              try {
                ...
                console.error(
                  `[Webhook] Error al leer archivo ${product.fileName}:`,
                  err,
                );
              }
      ...
      console.log(
        `[Webhook] Evento approved: Correo de aprobación enviado al cliente con ${payload.metadata?.items?.length || 0} items.`,
      );
    }
